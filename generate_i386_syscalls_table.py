...
def main():
    ...
    # URL to get linux_syscalls_table for i386
    URL = "https://faculty.nps.edu/cseagle/assembly/sys_call.html"

    # Syscall arg registers for 32-bit
    REGISTERS = ["%ebx", "%ecx", "%edx", "%esx", "%edi"]
    ...
    # Extract the table rows and process syscalls
    for row in table.find_all('tr')[1:]:
        ...
        for i in range(len(REGISTERS)):
            arguments[REGISTERS[i]] = cols[i+3].get_text(strip=True)
    ...
    output = []
    output.append("/* The code was generated using resources from https://faculty.nps.edu/cseagle/assembly/sys_call.html using the generation script (generate_i386_syscalls_table.py) */")
    ...
    for syscall_num, syscall_name, arguments in syscalls:
        ...
        enum_name_clean = enum_name.replace("(*NOTE)", "")
        comment = f"// {arguments}" if arguments else "// No arguments"
        output.append(f"    {enum_name_clean} = {syscall_num}, {comment}")
